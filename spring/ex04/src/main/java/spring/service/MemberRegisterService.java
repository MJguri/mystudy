...
public class MemberRegisterService {// 서블릿(Action 클래스)
	...
	// Dao 객체가 없으면 MemberRegisterService의 regist 기능을 작동시킬수 없다
	//    => MemberRegisterService 객체는 Dao객체에 '의존적'이다!!
	
	
	public MemberRegisterService(MemberDao memberDao) { // 외부에서 만들어진 객체를 입력받는다.
		this.memberDao = memberDao;						//   입력=> '주입'
		// 생성자
	}
